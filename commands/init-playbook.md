...
What the generated playbook provides: governance-neutral routing schema v2;
Codex-hosted scout as Desktop-controlled `host_local_cli / codex_read_only /
Luna`; native worker/executor; reciprocal external reviewers; and canonical
result schema v3 with controller-owned provenance plus provider_result. Template
generation itself is not real recheck evidence. This command preserves its
no-overwrite behavior and never modifies settings or hooks.
...
- provider 不接收、不輸出也不背書 authority fields。runner/controller 由已驗證
  packet 建立 immutable provenance；provider 只能產出 substantive
  provider_result。
...
| `codex_hosted` | Codex Desktop | scout＝Desktop-controlled `host_local_cli / codex_cli / codex_read_only / Luna`；worker/executor＝native Terra/Sol | `claude_cli` / `claude_read_only` |
...
- 兩個 host mode 的 `adapter_status = implemented`。Codex-hosted target 必須先用 `scripts/init_codex_host.py --target /absolute/path/to/target-repository` 安裝 20 個 repository-local 檔案；缺檔、衝突或未安裝時 fail closed。
...
- Codex-hosted mapping：`scout`→host-local CLI Luna；`worker`→native Terra；`executor`→native Sol。native scout TOML 已撤除；目前 embedded runtime 的 native scout sandbox 不構成 read-only 安全邊界。
...
| role | binding | 行為契約 |
|---|---|---|
| `feasibility_verifier` | `active_host_local_tier` | Claude-hosted 走 native tier；Codex-hosted 固定由 Desktop 明確派 `host_local_cli / codex_cli / codex_read_only / Luna`。後者屬 local tier，不是 external reviewer |
...
| `host_local_cli` | Codex Desktop 明確派發的 read-only Luna scout；需獨立 host-local execution authorization | 僅 codex_hosted / feasibility_verifier / scout |
...
| `codex_cli` | 非互動 `codex exec`，經 bounded runner 呼叫（Codex-hosted scout host-local path、Claude-hosted reviewer、或 opt-in headless implementation；三者授權不可互用） |
| `claude_cli` | 非互動 `claude -p`，經 bounded external-agent runner 呼叫（codex_hosted 的 reviewer） |

除精確的 Codex-hosted scout `host_local_cli` tuple 外，host-local tier 不得
映射到 CLI provider；新增其他組合需另一個 work package。
...
- 非 Codex-hosted scout 的 host-local tier 映射到 CLI、或同 provider 家族自審；
- 唯讀角色映射到可寫 profile；
...
- Codex scout 缺 packet＋CLI 雙重
  `ALLOW_HOST_LOCAL_CLI_INVOCATION`，或嘗試以 external reviewer authorization
  代替；兩種授權互不外溢；
- packet 缺 authoritative plan SHA、candidate SHA、target repository HEAD 或
  target status/dirty-state evidence；
