...
コントリビューション——特に README の翻訳や修正——を歓迎します。[CONTRIBUTING.md](CONTRIBUTING.md) を参照してください。核となるルール: コマンドとエージェントは英語で書かれ、**言語ミラーリングの指示を必ず保持しなければなりません**。これにより、アシスタントは常にユーザー自身の言語でユーザーと会話します。
